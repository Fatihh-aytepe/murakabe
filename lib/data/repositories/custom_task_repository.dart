...
class CustomTaskRepository {
  ...
  Future<List<CustomTaskModel>> getAllTasks() async {
    final rows = await _db.query('custom_tasks',
        where: null, orderBy: 'createdAt ASC');
  ...
  Future<void> markTaskCompleted(String taskId) async {
    ...
    if (existing.isEmpty) {
      await _db.insert('custom_task_completions', {
        'id': const Uuid().v4(),
        'taskId': taskId,
        'completedDate': today,
  ...
  Future<void> _scheduleTaskNotification(CustomTaskModel task) async {
    ...
    await NotificationService().scheduleCustomReminder(
      ...
      task.description.isNotEmpty ? task.description : 'Günlük görevin seni bekliyor',
      scheduledTime,
    );
  }

  int _taskNotifId(String taskId) {
    return 2000 + taskId.hashCode.abs() % 1000;
  }
}
